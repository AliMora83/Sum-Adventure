...
export function Hlotse() {
  ...
  return (
    <section id="hlotse" className="relative bg-ice py-26">
      <div className="mx-auto max-w-[1180px] px-7 lg:pl-[152px]">
        <div className="grid gap-15 md:grid-cols-[1.15fr_0.85fr]">
          <div className="reveal">
            ...
            <h2 className="type-display mt-5 text-[clamp(28px,3.9vw,44px)] text-surface-dark">
              Every journey becomes a story worth telling
            </h2>
            <p className="mt-5 max-w-[46ch] text-[16.5px] text-[#2F3E3C]">
          ...
          <dl className="reveal border-t-2 border-surface-dark pt-1">
            {ledger.map(([term, value]) => (
              <div
                key={term}
                className="flex justify-between gap-4 border-b border-teal-deep/20 py-4 text-[14.5px]"
              >
                <dt className="text-[#2F3E3C]">{term}</dt>
                <dd className="type-data whitespace-nowrap text-[12.5px] text-teal-deep">
                  {value}
                </dd>
              </div>
            ))}
